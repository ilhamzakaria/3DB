...
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <div class="d-flex flex-wrap align-items-center justify-content-between">
            ...
            <form method="get" action="<?= base_url('gudang/stiker') ?>" class="d-flex align-items-center mb-0 mt-2 mt-md-0">
        ...
        <?php if (!empty($start_date) || !empty($end_date)) : ?>
            <div class="mt-3">
                ...
                <a href="<?= base_url('gudang/stiker') ?>" class="text-danger small ml-2"><i class="fas fa-times"></i> Reset</a>
            </div>
        <?php endif; ?>
    </div>
                                ...
                                <td>
                                    <?php if (in_array(session()->get('role'), ['gudang', 'admin'], true)) : ?>
                                        <div class="d-flex gap-1 justify-content-center">
                                            ...
                                            <button type="button" class="btn btn-danger btn-sm ml-1" onclick="setDeleteModal(<?= $item['id'] ?>, '<?= esc($item['nama_stiker']) ?>')" data-toggle="modal" data-target="#modalDelete"><i class="fas fa-trash"></i></button>
                                        </div>
                                    <?php endif; ?>
                                </td>
                            </tr>
...
<!-- Modals same pattern as gilingangalon but names adjusted -->
<div class="modal fade" id="addDataModal" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header"><h5 class="modal-title">Tambah Stiker</h5><button type="button" class="close" data-dismiss="modal">&times;</button></div>
            <form action="<?= base_url('gudang/stiker/tambahData') ?>" method="post">
                <?= csrf_field() ?>
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6"><div class="form-group"><label>Nama Item</label><input type="text" class="form-control" name="nama_stiker" required></div></div>
                    </div>
...
<div class="modal fade" id="editDataModal" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header"><h5 class="modal-title">Edit Stiker</h5><button type="button" class="close" data-dismiss="modal">&times;</button></div>
            <form id="formEdit" method="post">
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6"><div class="form-group"><label>Nama Item</label><input type="text" class="form-control" name="nama_stiker" id="edit_nama" required></div></div>
                    </div>
                    <div class="form-group">
                        <label>Status</label>
...
<div class="filter-sidebar">
    ...
    <form method="get" action="<?= base_url('gudang/stiker') ?>">
        ...
        <div class="filter-footer">
            <a href="<?= base_url('gudang/stiker') ?>" class="btn btn-secondary flex-fill">Reset</a>
...
<script>
    ...
    function setEditModal(data) {
        ...
        $('#edit_nama').val(data.nama_stiker);
        $('#edit_status').val(data.status);
        $('#edit_kode').val(data.kode);
        $('#edit_jumlah').val(data.jumlah);
        $('#edit_shif').val(data.shif);
        $('#edit_lot').val(data.nomor_lot);
        $('#formEdit').attr('action', '<?= base_url('gudang/stiker/update') ?>/' + data.id);
    }
    function setDeleteModal(id, nama) {
        $('#delete_id').val(id);
        $('#delete_message').text('Hapus stiker "' + nama + '"?');
        $('#formDelete').attr('action', '<?= base_url('gudang/stiker/delete') ?>/' + id);
    }
</script>
