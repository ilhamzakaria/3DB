...
<?php
$statusLabels = ['menggiling' => 'Menggiling', 'bahan_masuk' => 'Bahan Masuk', 'bahan_keluar' => 'Bahan Keluar'];
$statusBadges = ['menggiling' => 'badge-warning', 'bahan_masuk' => 'badge-success', 'bahan_keluar' => 'badge-info'];
$jenisLabels = [
    'reject_preform' => 'Reject Preform',
    'bekuan_pet' => 'Bekuan PET',
    'bekuan_cap_galon' => 'Bekuan Cap Galon',
    'gilingan_screwcap' => 'Gilingan Screwcap'
];
    ...
    <div class="card-header py-3">
        <div class="d-flex flex-wrap align-items-center justify-content-between">
            <div class="d-flex align-items-center">
                ...
                <button class="btn btn-outline-primary btn-sm" onclick="toggleFilter(true)">
                    ...
                    <?php if (!empty($start_date) || !empty($end_date) || !empty($jenis_filter)) : ?>
                        <span class="badge badge-primary ml-1">!</span>
                    <?php endif; ?>
                </button>
            </div>

            <form method="get" action="<?= base_url('gudang/reject_produksi') ?>" class="d-flex align-items-center mb-0 mt-2 mt-md-0">
        ...
        <?php if (!empty($start_date) || !empty($end_date) || !empty($jenis_filter)) : ?>
            <div class="mt-3">
                ...
                <?php if (!empty($jenis_filter)) : ?><span class="filter-chip">Jenis: <?= esc($jenisLabels[$jenis_filter] ?? $jenis_filter) ?></span><?php endif; ?>
                <a href="<?= base_url('gudang/reject_produksi') ?>" class="text-danger small ml-2"><i class="fas fa-times"></i> Reset</a>
            </div>
        <?php endif; ?>
    </div>
                    ...
                    <?php if (!empty($data_list)) : ?>
                        <?php foreach ($data_list as $item) : ?>
                            <tr>
                                ...
                                <td><?= esc($item['nama_limbah'] ?? '-') ?></td>
                                <td><span class="badge badge-light border"><?= esc($jenisLabels[$item['jenis_limbah']] ?? $item['jenis_limbah']) ?></span></td>
                                ...
                                <td>
                                    <?php if (in_array(session()->get('role'), ['gudang', 'admin'], true)) : ?>
                                        <div class="d-flex gap-1 justify-content-center">
                                            ...
                                            <button type="button" class="btn btn-danger btn-sm ml-1" onclick="setDeleteModal(<?= $item['id'] ?>, '<?= esc($item['nama_limbah']) ?>')" data-toggle="modal" data-target="#modalDelete"><i class="fas fa-trash"></i></button>
                                        </div>
                                    <?php endif; ?>
                                </td>
                            </tr>
...
<div class="modal fade" id="addDataModal" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header"><h5 class="modal-title">Tambah Reject/Limbah</h5><button type="button" class="close" data-dismiss="modal">&times;</button></div>
            <form action="<?= base_url('gudang/reject_produksi/tambahData') ?>" method="post">
                <?= csrf_field() ?>
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6"><div class="form-group"><label>Nama Item</label><input type="text" class="form-control" name="nama_limbah" required></div></div>
                    </div>
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group"><label>Jenis Reject</label>
                                <select class="form-control" name="jenis_limbah">
                                    <?php foreach ($jenis_options as $opt) : ?><option value="<?= $opt ?>"><?= $jenisLabels[$opt] ?></option><?php endforeach; ?>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-6">
...
<div class="modal fade" id="editDataModal" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header"><h5 class="modal-title">Edit Reject/Limbah</h5><button type="button" class="close" data-dismiss="modal">&times;</button></div>
            <form id="formEdit" method="post">
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6"><div class="form-group"><label>Nama Item</label><input type="text" class="form-control" name="nama_limbah" id="edit_nama" required></div></div>
                    </div>
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group"><label>Jenis Reject</label>
                                <select class="form-control" name="jenis_limbah" id="edit_jenis">
                                    <?php foreach ($jenis_options as $opt) : ?><option value="<?= $opt ?>"><?= $jenisLabels[$opt] ?></option><?php endforeach; ?>
                                </select>
                            </div>
                        </div>
...
<div class="filter-sidebar">
    ...
    <form method="get" action="<?= base_url('gudang/reject_produksi') ?>">
        <div class="filter-body">
            ...
            <div class="form-group">
                ...
                <select name="jenis" class="form-control">
                    ...
                    <?php foreach ($jenis_options as $opt) : ?><option value="<?= $opt ?>" <?= $jenis_filter == $opt ? 'selected' : '' ?>><?= $jenisLabels[$opt] ?></option><?php endforeach; ?>
                </select>
            </div>
        </div>
        <div class="filter-footer">
            <a href="<?= base_url('gudang/reject_produksi') ?>" class="btn btn-secondary flex-fill">Reset</a>
...
<script>
    ...
    function setEditModal(data) {
        ...
        $('#edit_nama').val(data.nama_limbah);
        $('#edit_jenis').val(data.jenis_limbah);
        $('#edit_status').val(data.status);
        $('#edit_kode').val(data.kode);
        $('#edit_jumlah').val(data.jumlah);
        $('#edit_shif').val(data.shif);
        $('#edit_lot').val(data.nomor_lot);
        $('#formEdit').attr('action', '<?= base_url('gudang/reject_produksi/update') ?>/' + data.id);
    }
    function setDeleteModal(id, nama) {
        $('#delete_id').val(id);
        $('#delete_message').text('Hapus data "' + nama + '"?');
        $('#formDelete').attr('action', '<?= base_url('gudang/reject_produksi/delete') ?>/' + id);
    }
</script>
